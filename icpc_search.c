...
int main(int argc, char **argv) {
  int   i, j, ng=0, ig_now=0, g[9], do_dtc = 0;
  float f, f0, fstart[9], fdelta[9], fnow[9], fsign[9]={0};
  float x, y, A, B, C, fom, max_fom = -9999, max_f[9], fomm, fomt, max_fom_loop = -9999;
  ...
  *(strstr(ptitle, " ")) = 0;    // end of config file name
  ...
  while (fgets(line, 256, file_in) && ng < 6) {
    if (line[0] == '#') continue;  // comment
    ...
    if (line[0] == '-' && line[1] == 'g') {
      if (*(line+2) >= 'a' && *(line+2) <= 'z') {
        ...
        if (*(line+2) == 'b') i = 8; //   adjust both of the previous 2 parameters in anti-correlation
      } else {
        i = atoi(line+2);
      }
      if (i < 0 || i > 8) {
      ...
      g[ng] = i;
      if (2 != sscanf(line+3, "%f %f", &fstart[ng], &fdelta[ng])) {
  ...
  printf(" >>> ng = %d\n flag   min   max  incr\n", ng);
  for (i=0; i<ng; i++) {
    printf(" -g%1d  %5.1f %5.1f\n", g[i], fstart[i], fdelta[i]);
    fnow[i] = fstart[i];
  }
  ...
  while (1) {
    ...
    for (i=0; i<ng; i++) {
      if (g[i] < 8) fprintf(file_out, "%7.1f ", fnow[i]);
    }

    float dE = 10.0, dV = 200.0, curve = 2000.0/(dE*dV);
    while (fgets(line, 256, file_in)) {
      if ((c = strstr(line, "Estimated depletion voltage"))) {
        c = strstr(c, "= ");
        sscanf(c+2, "%f", &f);
        if (Vhigh < f) Vhigh = f;
        if (Vlow  > f) Vlow = f;
    ...
    if (max_fom < fomt) {
      fprintf(file_out, " <<<<<\n");
      max_fom = fomt;
      for (j=0; j<9; j++) max_f[j] = fnow[j];
      // if not, try something else, ...a0
    } else {
      fprintf(file_out, "\n");
      for (j=0; j<ng; j++) fnow[j] = max_f[j];
      if (fsign[ig_now] < 0) {
        fsign[ig_now] = 1;  // ... like incrementing rather than decrementing the current variable, ...
      } else {
        fsign[ig_now] = 0;  // ... or trying the next variable.
        if (++ig_now >= ng) {
          ig_now = 0;  // start again with first variable
          if (max_fom_loop > max_fom - 0.02) break;  // have found the "global" optimum
          max_fom_loop = max_fom;
        }
        fsign[ig_now] = -1;
      }
    }
    
    // change one of the geometry values
    j = ig_now;
    if (g[j] == 3 && fsign[j] < 0 && fnow[j] + fdelta[j]*fsign[j] < 0) fsign[j] = 1;
    if (g[j] == 8) {
      fnow[j-2] += fdelta[j-2]*fsign[j];
      fnow[j-1] -= fdelta[j-1]*fsign[j];
    } else {
      fnow[j] += fdelta[j]*fsign[j];
    }
  ...
  for (i=0; i<ng; i++) if (g[i] < 8) fprintf(file_out, " %s ", title[g[i]]);
  fprintf(file_out, "\n  ");
  for (i=0; i<ng; i++) if (g[i] < 8) fprintf(file_out, "%7.1f ", max_f[i]);
  fprintf(file_out, "\n\n");
  fclose(file_out);

  printf("\n Maximum FOM value is %.3f\n at", max_fom);
  for (i=0; i<ng; i++) if (g[i] < 8) printf(" %s ", title[g[i]]);
  printf("\n  ");
  for (i=0; i<ng; i++) if (g[i] < 8) printf("%7.1f ", max_f[i]);
  printf("\n\n");
